...
window.cartPageAmount= 0.00
        return(
            <>
            <div style={{ fontSize: `${2.3}rem`, fontWeight:600,margin: `${1}em 0 0 ${2}em` }}>
            CART
          </div>
          ...
          {this.props.cartfil.map(
                ...
                window.cartPageAmount = cartTotAmount.toFixed(2)
                    ...
                    return (
                      <>
                       <hr style={{width:`${90}%`,opacity:0.3,marginTop:`${4}rem`}}/>
                        <div className="cartPage_Container">
                           
                          <div>
                            <h2 className="cartpage_name">{name}</h2>
                            <h3 className="cartpage_brand">{brand}</h3>
                            <h4 className="cartpage_amount">{cartAmount.currency.symbol}{cartAmount.amount}</h4>
                          ...
                          >
                            ...
                            <button className="cartpage_quantityupdate" onClick={()=>this.props.dispatch(addCart({quanincre,textindex,quanincrename}))} >+</button>
                            <span style={{marginRight:`${4}rem`,marginTop:`${1}rem`,fontWeight:600,fontSize:`${1.3}rem`}} >{quantity}</span>
                            <button  className="cartpage_quantityupdate" onClick={()=>this.props.dispatch(addCart({quandecre,textindex,quandecrename}))} >-</button>
                          </div>
                          <div>
                            <img src={img} alt="cart_image" className="cartpage_image" />
                          </div>
                        </div>
                      </>
                    );
                ...
                <div style={{marginTop:`${1}rem`,marginLeft:`${3.5}rem`,fontSize:`${1.8}rem`,fontWeight:700,width:`${90}%`}}>
                  <span >Total:</span>
                  <span style={{float:"right"}}>{this.props.currency.symbol}{window.cartPageAmount}</span>
                </div>
          </>
        )
    }
